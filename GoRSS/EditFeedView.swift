...
struct EditFeedView: View {
    ...
    init(viewModel: FeedViewModel, source: FeedSource) {
        self.viewModel = viewModel
        self.source = source
        _urlString = State(initialValue: source.url.absoluteString)
        _nameString = State(initialValue: source.name ?? "")
    }
    ...
    private func saveChanges() async {
        ...
        do {
            try await viewModel.updateSource(source, newName: nameString, newUrl: url)
            isValidating = false
            dismiss()
        } catch {
            isValidating = false
            errorMessage = "无法验证该订阅源。请检查链接是否正确。\n\n详情: \(error.localizedDescription)"
            showError = true
        }
    }
}
